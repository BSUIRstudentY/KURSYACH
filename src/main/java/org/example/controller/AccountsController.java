...
public class AccountsController {
    ...
    private HBox createAccountBox(Account account) {
        ...
        box.setAlignment(javafx.geometry.Pos.CENTER); // Центрирование содержимого карточки
        ...
        // Название счета и accountNumber
        ...
        deleteButton.setOnMouseEntered(e -> deleteButton.setStyle("-fx-background-color: #c0392b; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 5 10; -fx-border-radius: 5; -fx-background-radius: 5; " +
                "-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.3), 8, 0, 0, 4);"));
        deleteButton.setOnMouseExited(e -> deleteButton.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 5 10; -fx-border-radius: 5; -fx-background-radius: 5; " +
                "-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.2), 5, 0, 0, 2);"));
        Map<String, Double> finalBalances = balances;
        deleteButton.setOnAction(e -> handleDeleteAccount(account, finalBalances));

        // Собираем всё вместе
        HBox contentBox = new HBox(20);
        contentBox.setAlignment(javafx.geometry.Pos.CENTER); // Центрирование содержимого внутри карточки
        contentBox.getChildren().addAll(nameBox, balancesBox);
        box.getChildren().addAll(contentBox, typeLabel, deleteButton);
        HBox.setHgrow(contentBox, javafx.scene.layout.Priority.ALWAYS);

        return box;
    }

    private void handleDeleteAccount(Account account, Map<String, Double> balances) {
        ...
        if (hasMoney) {
            ...
            // Если денег нет, удаляем без подтверждения
            performAccountDeletion(account);
        }
    }
    ...
    private void performAccountDeletion(Account account) {
        try {
            ...
            updateAccountsTable();
            errorLabel.setText("Счет успешно удален!");
        } catch (Exception e) {
            errorLabel.setText("Ошибка при удалении счета: " + e.getMessage());
